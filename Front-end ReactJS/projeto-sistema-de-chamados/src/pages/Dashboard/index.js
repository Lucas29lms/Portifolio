...
export default function Dashboard() {
    ...
    async function loadChamados() {
        ...
        await listaRef.limit(5)
            ...
            .catch((err) => {
                console.log('Erro ao carregar os chamados!', err)
    ...
    async function updateState(snapshot) {
        ...
        if (!isCollectionEmpty) {
            ...
            snapshot.forEach((doc) => {
                lista.push({
                    id: doc.id,
                    cliente: doc.data().customers,
                    clienteId: doc.data().clienteId,
                    assunto: doc.data().assunto,
                    status: doc.data().status,
                    dataCdastro: doc.data().created,
                    dataCdastroFormat: format(doc.data().created.toDate(), 'dd/MM/yyyy'),
                                ...
                                {chamados.map((item, index) => {
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td data-label="Cadastrado">{item.dataCdastroFormat}</td>
                                            <td data-label="#">
